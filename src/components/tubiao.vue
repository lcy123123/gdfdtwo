<template>
  ...
  <div style="width:168px;height:880px;position: absolute;top:0px;left:0px;">
    <!-- <router-view></router-view> -->
    <div class="left-z">
      <!-- 左面第一个图表 -->
      <div
        class="left-1 bottom-line"
        style="background-color:rgba(0, 3, 44, 0.5)"
      >
        <div class="div-btn">
        ...
        <div
          id="main-left-1"
          style="width:100%;height:calc((100vh - 120px)/2.7);position:absolute;top:-8px;left:2px"
        ></div>
      </div>
      <!-- 左面第二个图表 -->
      <div
        class="left-2 bottom-line"
        style="background-color:rgba(0, 3, 44, 0.5)"
      >
        <div class="div-btn">
        ...
        <div
          id="main-left-2"
          style="width:100%;height:calc((100vh - 120px)/2.7);position:absolute;top:-10px;left:10px"
        ></div>
      </div>
      <!-- 左面第三个图表 -->
      <div
        class="left-3 bottom-line"
        style="background-color:rgba(0, 3, 44, 0.5)"
      >
        <div class="div-btn">
    ...
    <div style="position:fixed;right:0px;top:0px;margin-right:10px">
      ...
      >
        ...
        <div
          id="main-left-4"
          style="width:100%;height:calc((100vh - 120px)/3.4);position:absolute;top:22px;right:0px"
        ></div>
      </div>
      <!-- 右面第二个图表 -->
      <div
        class="right-2 bottom-line"
        style="background-color:rgba(0, 3, 44, 0.5)"
      >
        <div class="div-btn">
          <span class="first-text">遥感数据对比</span
          ><span class="btn-1">×</span>
          <el-form>
            <el-form-item
              style="position:absolute;top:18px;right:22px;z-index:1"
            >
        ...
        <div
          id="main-left-5"
          style="width:100%;height:calc((100vh - 120px)/2.7);position:absolute;top:-10px;right:-12px"
        ></div>
      </div>
      <!-- 右面第三个图表 -->
      <div
        class="right-3 bottom-line"
        style="background-color:rgba(0, 3, 44, 0.5)"
      >
        <div class="div-btn">
          <span class="first-text">{{ fbsj }}</span
          ><span class="btn-1">×</span>
        </div>
        <div
          id="main-left-6"
    ...
    <!-- 站点名字 -->
    <span
      style="color:#79E5FD;position: fixed;bottom: calc(100vh/14.8);left:calc(100vh/3*1.9);"
      >{{ zdname }}</span
    >
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeValue(e) {
      ...
      this.getyaoGanShuJuDuiBi(this.ggpid);
      //调用 请求浮标数据对比方法 
      //在选择高度时对浮标数据和预报数据进行判断（浮标数据只有高度为10，预报数据有多中高度数据）
      if(this.fbsj=='浮标数据对比'){
        this.getfuBiaoShuJuDuiBi(this.ggpid);
      }else{
      this.getYuBaoShuJuDuiBi(this.ggpid);
    ...
    async getfengSuYueBianHua(gdyxValue, ggpid) {
      ...
      //请求接口 获取数据
      await this.$axios
        .post("/api/show/MonthlyVariationWindSpeed", {
          local: ggpid || 3,
          level: gdyxValue || 10
        })
        .then(res => {
    ...
    async getfengGongLvMiDuTongJi(gdyxValue, ggpid) {
      //请求接口 获取数据
      await this.$axios
        .post("/api/show/WindPowerDensity", {
          local: ggpid || 3,
          level: gdyxValue || 10
        })
        .then(res => {
    ...
    async getyouXiaoFengSu(gdyxValue, ggpid) {
      //请求接口 获取数据
      await this.$axios
        .post("/api/show/EffectiveWindSpeed", {
          local: ggpid || 3,
          level: gdyxValue || 10
        })
        .then(res => {
    ...
    async getyaoGanShuJuDuiBi(ggpid) {
      //请求接口 获取数据
      await this.$axios
        .post("/api/show/RemoteSensingData", {
          id: ggpid || 3,
          sat: this.wxvalue2 || "ascat"
        })
        .then(res => {
          //清除数组中上一次的值
          this.ldfsList = [];
    ...
    async getYuBaoShuJuDuiBi(ggpid) {
      ...
      //请求接口 获取数据
      await this.$axios
        .post("/api/show/ForecastRes", {
          local: ggpid || 3,
          level: this.gdyxValue || 10
        })
        .then(res => {
          //清空数组之前的值
          this.fbldfsList = [];
    ...
    //加载浮标数据对比
    fuBiaiShuJuDuiBi(fbldfsList, fbfsList, fbTime, intervallength) {
      //右侧折线图2
...
.gdxlk {
  ...
  /* top: 530px; */
  /* bottom: 5%; */
  bottom: calc(100vh / 14.8);
  /* right: 378px; */
  right: calc(100vh / 3 * 2);
  width: 150px;
